...
public static class SseUtils
{
    ...
    public static void SendPayload(string ev, SsePayload payload)
    {
        ...
        Task.Run(async () =>
        {
            var player = SseClient.Instance.Dalamud.ClientState.LocalPlayer;
            payload.Origin = player?.Name.TextValue;
            payload.OriginId = OriginId;

            payload.TerritoryTypeId = SseClient.Instance.Dalamud.ClientState.TerritoryType;
            ...
            await client.PostAsync($"{SseClient.Instance.Config.EndpointUrl}/collect/{ev}?token={SseClient.Instance.Config.Token}", content);
    ...
    {
        if (SseClient.Instance.Config.EnableDoNotDisturb)
        {
            return;
        }
        ...
        if (SseClient.Instance.Config.ShowIconOnReceivedMessages)
        {
        ...
        SseClient.Instance.Divination.Chat.EnqueueChat(entry);
    }
