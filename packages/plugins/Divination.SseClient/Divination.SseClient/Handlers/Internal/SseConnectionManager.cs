...
namespace Divination.SseClient.Handlers
{
    public class SseConnectionManager : IDisposable
    {
        ...
        public void Connect()
        {
            ...
            {
                SseClientPlugin.Instance.Divination.Chat.PrintError("SseServer の URL が設定されていません。接続先を設定で入力してください。");
                return;
            }

            eventSourceReader?.Dispose();
            eventSourceReader = new EventSourceReader(new Uri($"{SseClientPlugin.Instance.Config.EndpointUrl}/stream?token={SseClientPlugin.Instance.Config.Token}"));
        ...
        private void OnSseDisconnected(object sender, DisconnectEventArgs e)
        {
            ...
            {
                SseClientPlugin.Instance.Divination.Chat.PrintError("SseServer から切断されました。再接続を試みます。");
            }
