...
namespace Divination.DiscordIntegration
{
    public class Formatter
    {
        ...
        private Formatter()
        {
            player = DiscordIntegrationPlugin.Instance.Dalamud.ClientState.LocalPlayer ?? throw new InvalidDalamudDataProvidedException();
            territoryId = DiscordIntegrationPlugin.Instance.Dalamud.ClientState.TerritoryType;
        ...
        private void UpdateJob()
        {
            ...
            if (DiscordIntegrationPlugin.Instance.Config.ShowJobCustomStatusEmoji)
            {
        ...
        {
            var territoryType = DiscordIntegrationPlugin.Instance.Dalamud.DataManager.GetExcelSheet<TerritoryType>()!.GetRow(territoryId);
        ...
        {
            var target = DiscordIntegrationPlugin.Instance.Dalamud.TargetManager.Target;
        ...
        {
            var partyListCount = DiscordIntegrationPlugin.Instance.Dalamud.PartyList.Length;
        ...
        {
            var condition = DiscordIntegrationPlugin.Instance.Dalamud.DataManager.GetExcelSheet<ContentFinderCondition>()!.FirstOrDefault(x => x.TerritoryType.Row == territoryId);
            if (condition != null)
            {
                Duty = condition.Name;
        ...
        {
            var combat = DiscordIntegrationPlugin.Instance.Dalamud.Condition[ConditionFlag.InCombat];

            inCombat = combat && (!DiscordIntegrationPlugin.Instance.Config.RequireTargetingOnCombat || isTargeting);
        }
        ...
        private void UpdateTimestamp()
        {
            ...
            if (shouldResetTimer && DiscordIntegrationPlugin.Instance.Config.ResetTimerOnAreaChange)
            {
        ...
        private void UpdateOnlineStatus()
        {
            ...
            if (DiscordIntegrationPlugin.Instance.Config.ShowOnlineStatusCustomStatusEmoji && (!DiscordIntegrationPlugin.Instance.Config.ShowJobCustomStatusEmoji ||
                                                      inDuty &&
        ...
        public string GetTemplate(string templateKey)
        {
            switch (templateKey)
            {
                case "details":
                    if (inCombat)
                    {
                        return DiscordIntegrationPlugin.Instance.Config.DetailsInCombatFormat;
                    }
                    else if (inDuty)
                    {
                        return DiscordIntegrationPlugin.Instance.Config.DetailsInDutyFormat;
                    }
                    else if (isOnline)
                    {
                        return DiscordIntegrationPlugin.Instance.Config.DetailsInOnlineFormat;
                    }
                    else
                    {
                        return DiscordIntegrationPlugin.Instance.Config.DetailsFormat;
                    }
                case "state":
                    return DiscordIntegrationPlugin.Instance.Config.StateFormat;
                case "small_image_text":
                    return DiscordIntegrationPlugin.Instance.Config.SmallImageTextFormat;
                case "large_image_text":
                    return DiscordIntegrationPlugin.Instance.Config.LargeImageTextFormat;
                case "custom_status":
                    return inDuty ? DiscordIntegrationPlugin.Instance.Config.CustomStatusInDutyFormat : DiscordIntegrationPlugin.Instance.Config.CustomStatusFormat;
                default:
        ...
        public static RichPresence? CreatePresence()
        {
            ...
            return new RichPresence
            {
                ...
                Assets = new Assets
                {
                    ...
                    SmallImageText = DiscordIntegrationPlugin.Instance.Config.ShowJobSmallImage ? instance.Format("small_image_text") : null,
                    LargeImageKey = instance.largeImageKey,
                    LargeImageText = DiscordIntegrationPlugin.Instance.Config.ShowLoadingLargeImage ? instance.Format("large_image_text") : null
                },
