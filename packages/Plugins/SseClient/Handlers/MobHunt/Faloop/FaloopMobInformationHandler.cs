    ...
    public class FaloopMobInformationHandler : ISsePayloadReceiver, IDisposable
    {
        ...
        public bool CanReceive(string eventId)
        {
            return eventId == "faloop_message" &&
                   (SseClientPlugin.Instance.Config.ReceiveFaloopSpawnMessages
                    || SseClientPlugin.Instance.Config.ReceiveFaloopKillMessages
                    || SseClientPlugin.Instance.Config.ReceiveFaloopSightMessages);
        }
        ...
        {
            if (!SseClientPlugin.Instance.Config.ReceiveFaloopSpawnMessages)
            {
                return;
            }
            ...
            {
                Type = SseClientPlugin.Instance.Config.MobHuntFaloopSpawnMessagesType,
        ...
        {
            if (!SseClientPlugin.Instance.Config.ReceiveFaloopKillMessages)
            {
                return;
            }
            ...
            SseUtils.PrintSseChat(new XivChatEntry
            {
                Type = SseClientPlugin.Instance.Config.MobHuntFaloopKillMessagesType,
        ...
        private void OnFaloopMobKilledUnknown(FaloopPayload.MobStatus status, FaloopPayload.MobStatus.WindowStatus window)
        {
            ...
            if (SseClientPlugin.Instance.Config.FilterOldMobKills && DateTime.UtcNow - DateTimeOffset.FromUnixTimeSeconds(window.StartedAt/1000).LocalDateTime > TimeSpan.FromMinutes(10))
            {
                return;
            }
        ...
        {
            if (!SseClientPlugin.Instance.Config.ReceiveFaloopSightMessages)
            {
                return;
            }
            ...
            SseUtils.PrintSseChat(new XivChatEntry
            {
                Type = SseClientPlugin.Instance.Config.MobHuntFaloopSightMessagesType,
        ...
        private SeString CreateMapLink(int mobId, string location, int? instance)
        {
            ...
            var territory = SseClientPlugin.Instance.Dalamud.DataManager
