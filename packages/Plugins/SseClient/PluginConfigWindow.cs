...
namespace Divination.SseClient
{
    public class PluginConfigWindow : ConfigWindow<PluginConfig>
    {
        ...
        {
            if (ImGui.Begin($"{SseClientPlugin.Instance.Name} 設定", ref IsOpen, ImGuiWindowFlags.HorizontalScrollbar))
            {
                if (ImGui.BeginTabBar($"#{SseClientPlugin.Instance.Name}-tabs"))
                {
                ...
                if (ImGui.Button("Save & Close"))
                {
                    ...
                    SseClientPlugin.Instance.Dalamud.PluginInterface.SavePluginConfig(Config);
                    PluginLog.Information("Config saved.");

                    SseClientPlugin.IsConfigChanged = true;
                    SseClientPlugin.ConnectToSse();
                }
        ...
        private void CreateEndpointConfigTab()
        {
            ...
            {
                ImGui.Text($"現在の状態: {(SseClientPlugin.IsDisconnected ? "未接続" : "接続中")}");
